...
const showAllSongsInThisPlaylist = (request,response) => {
  ...
  let query = "SELECT playlist_song,songs.id,songs.title FROM songs INNER JOIN playlists_songs ON (songs.id = playlists_songs.song_id) WHERE playlists_songs.playlist_id = '" + request.params.index + "';";
  pool.query(query,(err,result) =>{
    if(err){
      console.log("ERROR!",err);
      response.status(500).send("error");
